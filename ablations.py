...
def mc_dropout_scores_from_loader(data_loader, model, device, n_iterations=10, eps=1e-7):
    """
    蒙特卡洛采样 (MC Dropout): 通过多次前向传播计算预测的方差或平均熵。
    """
    ...
    with torch.no_grad():
        for batch_idx, (data, _) in enumerate(data_loader):
            try:
                ...
                # 计算平均概率 [B, C, H, W]
                avg_probs = batch_probs.mean(dim=0).clamp(eps, 1.0 - eps)
                
                # 计算预测熵 (Predictive Entropy)
                entropy_map = -torch.sum(avg_probs * avg_probs.log(), dim=1) # [B, H, W]
                batch_scores = entropy_map.view(entropy_map.size(0), -1).mean(dim=1)

                scores_all.append(batch_scores.cpu().numpy())
                overall_uncertainty += entropy_map.sum().item()
            except Exception as e:
                logging.error(f"Error processing batch {batch_idx} in MC Dropout: {str(e)}")
                continue

    if len(scores_all) == 0:
        return np.empty((0,), dtype=np.float32), 0.0

    return np.concatenate(scores_all, axis=0), overall_uncertainty


def entropy_scores_from_loader(data_loader, model, device, eps=1e-7):
    """
    基于熵的采样策略 (Vanilla Entropy Sampling)。
    """
    model.eval()
    scores_all = []
    overall_uncertainty = 0.0
    ...
    with torch.no_grad():
        for batch_idx, (data, _) in enumerate(data_loader):
            try:
                ...
                entropy_map = -torch.sum(probs * probs.log(), dim=1) # [B, H, W]
                batch_scores = entropy_map.view(entropy_map.size(0), -1).mean(dim=1)

                scores_all.append(batch_scores.cpu().numpy())
                overall_uncertainty += entropy_map.sum().item()
            except Exception as e:
                logging.error(f"Error processing batch {batch_idx} in Entropy Sampling: {str(e)}")
                continue
...
def main():
    ...
    # =================== 超参数 ===================
    # sampling_mode: 'random', 'dcums', 'mc_dropout', 'entropy'
    sampling_mode = 'entropy'              
    nb_experiments = 1                   # 实验次数
    nb_active_learning_iter = 12         # 主动学习轮数
    AL_BUDGET_RATIO = 0.02              # 每一轮从未标注池中选 8%
    MIN_ADD = 1                         # 每轮至少选 1 个样本
    max_iters = 800                          # max_iters
    ...
    swanlab.init(
        project="active_loop",
        workspace="xuecheng",
        name=f"{model_name}_{sampling_mode}_{AL_BUDGET_RATIO*100}_{dataname}%",
        config={
            "iters": max_iters,
            "lr": LEARNING_RATE,
    ...
    for r in range(nb_experiments):
        ...
        for i in range(nb_active_learning_iter):
            ...
            logging.info(
                "AL Round %d | Unlabeled pool size: %d | Budget (10%%): %d | Mode: %s",
                i + 1, N_unlabeled, K, sampling_mode
            )


            model_path = f'ablations/{sampling_mode}/{dataname}/{model_name}/{AL_BUDGET_RATIO}/{model_name}_iter_{i}.pth'
            data_type = sampling_mode

            # 第 0 轮：从 base 模型开始
            if i == 0:
                print(f"Loading base model for {model_name}...")
                if model_name == 'attentionunet' or model_name == 'DCUMS':
                    state_dict = torch.load(
                        "pth/DCUMS/keviar-seg/attentionunet/0.1/TransresUNet_cls.pth",
                        weights_only=True
                    )
                else:
                    state_dict = torch.load(
                        'pth/DCUMS/keviar-seg/attentionunet/0.1/UNet.pth',
                        map_location='cuda',
                        weights_only=True
                    )
            ...
            if sampling_mode == 'random':
            ...
            elif sampling_mode == 'dcums':
                logging.info("使用 DCUMS 计算未标注集的不确定性 ...")
                ...
                if mean_uncertainty == 0: mean_uncertainty = 1e-8
            ...
            elif sampling_mode in ['mc_dropout', 'entropy']:
                logging.info(f"使用 {sampling_mode} 计算未标注集的不确定性 (普通 Top-K 采样) ...")
                if sampling_mode == 'mc_dropout':
                    uncertainty_scores, overall_uncertainty_score = mc_dropout_scores_from_loader(
                        unlabelled_loader, model, DEVICE, n_iterations=10
                    )
                else: # entropy
                    uncertainty_scores, overall_uncertainty_score = entropy_scores_from_loader(
                        unlabelled_loader, model, DEVICE
                    )
                
                # 普通采样：选择得分最高的 K 个样本
                to_be_annotated_indices = np.argsort(uncertainty_scores)[-K:]
                logging.info("第 %d 次主动学习迭代的整体不确定性得分为 %f", i+1, overall_uncertainty_score)
            else:
            ...
            logging.info("样本索引 %s 被选为采样样本", str(to_be_annotated_indices))
