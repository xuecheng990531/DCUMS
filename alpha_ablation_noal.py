...
def main():
    ...
    swanlab.init(
        project="alpha_ablation_noal",
        name=f"TransResUNet_alpha{ALPHA}",
        workspace="xuecheng",
        config={
    ...
    os.makedirs(f'experiment_logs/alpha_ablation_noal/alpha{ALPHA}', exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s',
        filename=f'experiment_logs/alpha_ablation_noal/alpha{ALPHA}/training_output.txt',
        filemode='w'
    )
    ...
    model = TransUNet(
        ...
        block_num=4, patch_dim=16, class_num=2
    ).to(DEVICE)
    ...
    pbar = tqdm(total=TOTAL_ITRS, desc=f"Training (alpha={ALPHA})", unit="iter")
    ...
    np.save(f'experiment_logs/alpha_ablation_noal/alpha{ALPHA}/quality_history.npy', np.array(quality_history))

    print("\n" + "="*50)
    print(f"Alpha = {ALPHA} (ub=0.2) 消融实验结果")
    print("="*50)
