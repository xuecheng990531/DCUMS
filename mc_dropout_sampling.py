# 基于 MC-Dropout 的主动学习采样主程序
# - 以 random_sampling.py 为模板
# - 采样策略：MC-Dropout（支持 predictive entropy / BALD）
...
# Optimizer fallback（部分 torch stub 可能不从 torch.optim 直接导出 AdamW）
try:
...
def _enable_dropout_only(model: nn.Module) -> int:
    """将模型置于 eval，但让 Dropout 层保持 train，以进行 MC-Dropout 推理。
...
@torch.no_grad()
def mc_dropout_uncertainty_scores_from_loader(
    ...
    """对未标注池计算 MC-Dropout 不确定性分数。

    Args:
        data_loader: 未标注集 loader（shuffle=False，保证顺序与 unlabelled_img_paths 对齐）
        model: 分割模型，输出 logits: [B,C,H,W]
        device: 'cuda' or 'cpu'
        mc_passes: Monte Carlo 前向次数（越大越稳定，但越慢）
        score_type:
            - 'entropy': predictive entropy（H[p̄]）
            - 'bald': mutual information（H[p̄] - E[H[p_t]]）
        eps: 防止 log(0)

    Returns:
        scores: np.ndarray shape [N]
        overall_uncertainty: float（map 级总和，仅用于监控）
    """
    ...
    n_dropout = _enable_dropout_only(model)
    if n_dropout == 0:
        logging.warning(
            "模型中未检测到 Dropout 层，MC-Dropout 将退化为确定性推理（分数可能近似常数）。"
        )

    score_type = str(score_type).lower().strip()
    if score_type not in {"entropy", "bald"}:
        raise ValueError("score_type must be one of: {'entropy','bald'}")
    ...
    for batch_idx, (data, _) in enumerate(data_loader):
        ...
        mean_probs = probs_mc.mean(dim=0)  # [B,C,H,W]

        # H[p̄]
        entropy_mean = -(mean_probs * mean_probs.log()).sum(dim=1)  # [B,H,W]

        if score_type == "entropy":
            score_map = entropy_mean
        else:
            # E[H[p_t]]
            entropy_each = -(probs_mc * probs_mc.log()).sum(dim=2)  # [T,B,H,W]
            expected_entropy = entropy_each.mean(dim=0)  # [B,H,W]
            score_map = entropy_mean - expected_entropy  # [B,H,W]

        batch_scores = score_map.view(score_map.size(0), -1).mean(dim=1)  # [B]
        scores_all.append(batch_scores.detach().cpu().numpy())
        overall_uncertainty += float(score_map.sum().item())

        # 清理显存
        del data, probs_mc, mean_probs, entropy_mean, score_map, batch_scores
        if device == "cuda":
            torch.cuda.empty_cache()

    # 恢复模型模式（避免后续流程意外保持 dropout-only 状态）
    if prev_training:
        model.train()
    else:
        model.eval()
...
def main():
    ...
    # 训练 batch_size 与未标注池 scoring batch_size 分开（MC 评估更吃显存）
    TRAIN_BATCH_SIZE = 12
    UNLAB_BATCH_SIZE = 4

    # ===== MC-Dropout params =====
    MC_PASSES = 10
    MC_SCORE_TYPE = 'bald'  # 'bald' 或 'entropy'

    # =================== SwanLab ===================
    swanlab.init(
        project="mc_dropout_active_loop",
        workspace="xuecheng",
        name=f"unet2d_mc_dropout_{MC_SCORE_TYPE}",
        config={
            "iters": max_iters,
            "lr": LEARNING_RATE,
            ...
            "mc_passes": MC_PASSES,
            "mc_score_type": MC_SCORE_TYPE,
            "budget_ratio": AL_BUDGET_RATIO,
        },
    )

    # =================== Logging ===================
    learning_type = 'mc_dropout'
    ...
    logging.info("=> GO GO GO 出发咯 (MC-Dropout Sampling)")
    ...
    for r in range(nb_experiments):
        ...
        for i in range(nb_active_learning_iter):
            ...
            logging.info(
                "AL Round %d | Unlabeled: %d | MC-Dropout Budget: %d",
                i + 1,
                N_unlabeled,
                K,
            )
            ...
            logging.info(
                "计算未标注池 MC-Dropout 不确定性 (type=%s, passes=%d) ...",
                MC_SCORE_TYPE,
                MC_PASSES,
            )
            uncertainty_scores, overall_uncertainty = mc_dropout_uncertainty_scores_from_loader(
                unlabelled_loader,
                model,
                DEVICE,
                mc_passes=MC_PASSES,
                score_type=MC_SCORE_TYPE,
            )
                ...
                try:
                    ...
                    if do_validate:
                        ...
                        if val_log['dice'] > best_dice:
                            model_path = (
                                f'pth/mc_dropout/{model_name}/{AL_BUDGET_RATIO}/'
                                f'{model_name}_{MC_SCORE_TYPE}_mc{MC_PASSES}_iter_{i}.pth'
                            )
